...
const FEED_MERCHANTS = ["67974", "8626", "16673", "16809", "8794", "104933", "30295"] as const;
...
export const useAllAwinMerchants = (query: string, enabled: boolean) => {
  ...
  const m6 = useAwinMerchantProducts(FEED_MERCHANTS[5], query, enabled);
  const m7 = useAwinMerchantProducts(FEED_MERCHANTS[6], query, enabled);

  const products: AwinMerchantProduct[] = useMemo(
    () => [...m1.products, ...m2.products, ...m3.products, ...m4.products, ...m5.products, ...m6.products, ...m7.products],
    [m1.products, m2.products, m3.products, m4.products, m5.products, m6.products, m7.products],
  );

  const loading = m1.loading || m2.loading || m3.loading || m4.loading || m5.loading || m6.loading || m7.loading;
