...
export const useAllAwinMerchants = (query: string, enabled: boolean) => {
  ...
  const products: AwinMerchantProduct[] = useMemo(
    () => [...m1.products, ...m2.products, ...m3.products, ...m4.products, ...m5.products, ...m6.products],
    [m1.products, m2.products, m3.products, m4.products, m5.products, m6.products],
  );

  const loading = m1.loading || m2.loading || m3.loading || m4.loading || m5.loading || m6.loading;
